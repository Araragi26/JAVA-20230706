...
// 접근제어자 반환타입 메서드명(매게변수) { 행동로직 }

// 접근제어자 반환타입 메서드명(매게변수) : 선언부
// { 행동로직 } : 구현부

// 학생 : 학번, 이름, 나이, 전화번호, 수업, 성적 / 공부함, 시험, 잠을잠
class Student {
	
	// 클래스 변수 : 모든 인스턴스가 공통으로 값을 가지는 변수
	//            인스터스 생성없이 클래스로 바로 접근이 가능
	...
	//                반드시 인스터스가 생성된 후에 사용 가능
	void study() {
		int time = 4;
		System.out.println(course + "을 공부한다.");
	}
	
	void test() {
		System.out.println(course + "을 시험친다.");
	}
...
class MyMath {
    ...
    static double frustumOfACone(double r1, double r2, double h) {
		double result = (PIE * 3.14 * h * (r1* r1 + r1 * r2 + r2 * r2)) / 3;
		return result;
	}
    
	static double rightCircularConeV(double r, double h) {
		double result = (PIE * 3.14 * r * r * h) / 3;
		return result;
	}
}
...
public class ClassAndObject {
	...
	public static void main(String[] args) {
		...
		// 특정 클래스로 생성된 객체 '특정 클래스의 인스턴스' 라고 부름
