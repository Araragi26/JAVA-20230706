...
public class VariableAndType {
	...
	public static void main(String[] args) {
		...
		// int variable!;
		// 2) 문법상의 키워드 사용 불가능
		// int int; = 불가능, int int1 = 가능
		// 3) 변수명의 첫 글자가 숫자일 수 업음
		// int 1number;
		// 5) 변수명은 대소문자를 구분함
		// int Number = 100;
		...
		// 명명 규칙 (일반적)
		// 1. Camel Case :
		// 뛰어쓰기 위치를 표현하기 위하여 뛰어쓰기 뒤에오는 단어 첫 문자를 대문자로 표기
		...
		// lower_Snake_Case : 안쓰임 (JSP에서 java 변수와 javascript 변수를 구분짓기 위한 javascript의
		// 변수명)
		// UPPER_SNAKE_CASE : 상수
		...
		// 명명 규칙은 기본 변수 명명 규치 + UPPER_SNAKE_CASE
		...
		// 상수를 사용하는 이유
		// 1. 값의 보호 : 상수로 지정되어있는 데이터를 변경할 수 없도록 강제로 막고
		...
		// 정수형 타입 : 정수만 담을 수 있는 데이터 타입 (byte, short, int, long)
		// - byte : 1byte (8bit)의 메모리 공간을 차지함, 범위는 -128 ~ 127
		byte byteVariable;
		byteVariable = 127;
		byteVariable = -128;
		// byteVariable = 128; 범위를 초과하는 범위는 안됨

		// - short : 2byte (16bit)의 메모리 공간을 차지함, 범위는 -32,768 ~ 32,767
		short shortVariable;
		shortVariable = -32768;
		shortVariable = 32767;
		// shortVaraible =32768; 범위를 초과하는 범위는 안됨

		// - int : 4byte (16bit)의 메모리 공간을 차지함,
		// 범위는 -2,147,483,648(-2^31) ~ 2,147,483,647(2^31-1)
		int intVariable;
		intVariable = -2_147_483_648;
		intVariable = 2_147_483_647;
		// intVaraible = 2_147_483_648; 범위를 초과하는 범위는 안됨

		// - long : 8byte (32bit)의 메모리 공간을 차지함,
		// 범위는 -9,223,372,036,854,775,808(-2^63) ~ 9,223,372,036,854,775,807(2^63-1)
		long longVariable;
		longVariable = -2_147_483_648L;
		longVariable = 2_147_483_647L;

		// 실수형 타입 : 실수와 정수를 모두 담을 수 있는 데이터 타입

		// - float : 4byte (32bit)의 메모리 공간을 차지함
		// 소수점 6 ~ 7 자리에서 오차 발생
		float floatVariable;
		floatVariable = 3.14151617F; // float형의 경우 뒤에 'F' 또는 'f'를 붙여준다, 기본적으로 대문자 F를 사용함
		System.out.println(floatVariable);

		// - double : 8byte (64bit)의 메모리 공간을 차지함
		...
		// - char : 2byte (16bit)의 메모리 공간을 차지함
		// 문자 하나만 담을 수 있음, 아스키코드, 유니코드 또한 담을 수 있음
		char charVariable;
		charVariable = 'A';
		System.out.println(charVariable);
		charVariable = 97; // 아스키코드값 소문자 'a'
		System.out.println(charVariable);

		// 논리형 타입 : 논리 값을 담을 수 있는 데이터 타입 true, false의 값을 가질 수 있음
		// - boolean : 1byte (8bit)의 메모리 공간을 차지함
		...
		// 자동 형변환: 같은 타입일 때 작은 데이터 타입을 큰 데이터 타입으로 형변환 할떄는 자바가 알아서 형변환을 진행함
		byteVariable = 10;
		...
		// 강제 형변환 : 다른 타입이거나 큰 데이터 타입을 작은 데이터 타입으로 형변환할 떄는 개발자가 명시적으로 형변환을 해야함
		...
		// 실제 데이터가 저장되는 것이 아니라 데이터 배열이 저장된 첫번째 요소의 주소가 저장되는 것이다
		...
		// String 클래스는 문자의 배열을 개발자가 조금 더 쉽게 사용할 수 있도록 도움을 주는 Java에서 재공해주는 참조형 데이터 타입
		...
		// String 참조 타입의 포함하는 기능(String의 주요 메서드)
		...
		boolean isEquals = longComment1.equals(longComment2);
		System.out.println(isEquals);
		
		// 문자열 길이 : lenght(문자열변수); 기능을 사용
		int commentLenght = longComment.length();
		System.out.println(commentLenght);
		
		// 문자열 찾기 : indexOf(문자열변수); 기능을 사용
		...
		// 참조형 데이터 타입의 null
		// 참조형 데이타 타입의 변수의 경우는 그 무엇도 참조하지 않는다는 'null' 값을 가질 수 있음
		stringVariable = null;
