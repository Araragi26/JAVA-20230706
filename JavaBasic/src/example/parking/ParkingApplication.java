...
public class ParkingApplication {
	...
	public static void main(String[] args) {
		...
		// 층별 여유 공간
		int[] layerFreeSpace = new int[10];
		for (int layer = 0; layer < parkingSpaces.length; layer++) {
			for (boolean space : parkingSpaces[layer]) {
				if (!space)
					layerFreeSpace[layer]++;
			}
		}
		for (int layer = 0; layer < layerFreeSpace.length; layer++)
			System.out.println(layer + 1 + "층 여유 공간 : " + layerFreeSpace[layer]);
		...
		if (type != null && !type.equals("경차") && !type.equals("중대형") && !type.equals("SUV")) {
			System.out.println("경차, 중대형, SUV 중 하나를 입력하세요.");
			return;
		}

		// 입차시간 검증
		if (getIntime < 0 || getIntime > 24) {
			System.out.println("입차 시간 0 ~ 24 사이어야 합니다.");
			return;
		}
		...
		typse[parkingLayer][parkingSpace] = type;
		...
		// 요금 (출차시간 - 입차시간) * 차량별 금액
		int typeAmount = typse[foundLayer][foundSpace].equals("경차") ? 1000
				: typse[foundLayer][foundSpace].equals("중대형") ? 1500 : 2000;
		...
		typse[foundLayer][foundSpace] = null;
